#!/usr/bin/env python3
"""
Mock Data Generator for Event Horizon Database

This script generates realistic mock data for the Event Horizon Planner database,
matching the frontend expectations and ensuring all relationships are properly maintained.
"""
...
class MockDataGenerator:
    ...
    def __init__(self):
        ...
        self.project_names = [
            "Event Horizon Database Migration",
        ...
        self.project_descriptions = [
            "Migrate from Notion to PostgreSQL for better performance and n8n integration",
            "Optimize existing n8n workflows for better performance",
            "Update all API documentation to reflect new database schema",
            "Redesign the frontend with modern UI/UX principles",
    ...
    def clear_all_data(self):
        """Clear all existing data in proper order to respect foreign key constraints"""
    ...
    def generate_all_data(self):
        ...
        project_ids = self.generate_projects(3)
        if not project_ids:
            print("Failed to generate projects. Aborting.")
            return False
        
        goal_ids = self.generate_goals(project_ids, 3)
        if not goal_ids:
            print("Failed to generate goals. Aborting.")
            return False
        
        task_ids = self.generate_tasks(goal_ids, 3)
        if not task_ids:
            print("Failed to generate tasks. Aborting.")
            return False
...
def main():
    ...
    print("Event Horizon Mock Data Generator")
    print("=" * 50)
